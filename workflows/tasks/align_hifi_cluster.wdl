...
task alignClusteredHifiReads {
    ...
    command <<<
        ...
        if [ $(samtools view ~{file_label}_clustered_hifi_to_reference_alignment.bam | wc -l) -ge 1 ]
        then
            pbaa bampaint \
                ~{pbaa_read_info} \
                ~{file_label}_clustered_hifi_to_reference_alignment.bam \
            ...
            samtools flagstat \
                ~{file_label}_clustered_hifi_to_reference_alignment.bam > ~{file_label}_clustered_hifi_to_reference_alignment_painted_flagstat.txt
            
            samtools idxstat \
                ~{file_label}_clustered_hifi_to_reference_alignment.bam > ~{file_label}_clustered_hifi_to_reference_alignment_idx.txt
